...
var tokenBalancesCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		tokenID, _ := types.BigFromString(cctx.String("tokenID"))
		...
		if !tokenID.Nil() && !tokenID.LessThan(big.Zero()) && !ownerAddress.Empty() {
			...
			fmt.Printf("owner %s has %v token with tokenID %v.\n", ownerAddress, tokenAmount, tokenID)
		} else if (tokenID.Nil() || tokenID.LessThanEqual(big.Zero())) && !ownerAddress.Empty() {
			tokenAmountsByAddr, err := api.StateTokenBalanceByAddr(ctx, ownerAddress, types.EmptyTSK)
			if err != nil {
				return err
			}
...
var tokenCreateCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		params, err := actors.SerializeParams(&token3.CreateTokenParams{
			TokenURI: uri,
			ValueInit: amount,
		})
		if err != nil {
		...
		msg := &types.Message{
			...
			Method:     builtin3.MethodsToken.Create,
			Params:     params,
		}
...
var tokenMintBatchCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		msg := &types.Message{
			...
			GasFeeCap:  gfc,
			GasLimit:   cctx.Int64("gas-limit"),
			Method:     builtin3.MethodsToken.MintBatch,
			Params:     params,
		}
...
var tokenSetURICmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		params, err := actors.SerializeParams(&token3.ChangeURIParams{
			TokenID: tokenID,
			NewURI: newURI,
		})
		if err != nil {
		...
		msg := &types.Message{
			...
			GasFeeCap:  gfc,
			GasLimit:   cctx.Int64("gas-limit"),
			Method:     builtin3.MethodsToken.ChangeURI,
			Params:     params,
		}
	...
	Action: func(cctx *cli.Context) error {
		...
		if len(tokenIDs) == 1 {
			...
			msg = &types.Message{
				...
				GasFeeCap:  gfc,
				GasLimit:   cctx.Int64("gas-limit"),
				Method:     builtin3.MethodsToken.SafeTransferFrom,
				Params:     params,
			}
		} else {
			params, err := actors.SerializeParams(&token3.SafeBatchTransferFromParams{
				AddrFrom: addrFromID,
				AddrTo: addrToID,
				TokenIDs: tokenIDs,
				Values: amounts,
			...
			msg = &types.Message{
				...
				GasFeeCap:  gfc,
				GasLimit:   cctx.Int64("gas-limit"),
				Method:     builtin3.MethodsToken.SafeBatchTransferFrom,
				Params: 	params,
			}
		}
...
var tokenApprovesSetCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		msg := &types.Message{
			...
			GasFeeCap:  gfc,
			GasLimit:   cctx.Int64("gas-limit"),
			Method:     builtin3.MethodsToken.SetApproveForAll,
			Params:     params,
		}
